...
public class LoginService {
	...
	public String login(LoginCommand loginCommand, HttpSession session, Model model) throws Exception {
		...
		if (passwordEncoder.matches(loginCommand.getLoginPw(), lists.get(0).getMemPw())) {
			authInfo = new AuthInfo(lists.get(0).getMemId(), lists.get(0).getMemEmail(), lists.get(0).getMemName());
